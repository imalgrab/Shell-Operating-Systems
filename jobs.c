...
bool resumejob(int j, int bg, sigset_t *mask) {
  if (j < 0) {
    for (j = njobmax - 1; j > 0 && jobs[j].state == FINISHED; j--) continue;
  }

  if (j >= njobmax || jobs[j].state == FINISHED) return false;

  killpg(jobs[j].pgid, SIGCONT);
  jobs[j].state = RUNNING;

  if (!bg) {
    if (jobs[0].pgid == 0) { /* if fg jobs slot free move */
      movejob(j, FG);
      monitorjob(mask);
    }
  }
